...
echo $((60 * 1000000)) > /sys/module/damon_reclaim/parameters/min_age

rm -rf ./report/rec-custom-$DATE/
mkdir -p ./report/rec-custom-$DATE/

cat /proc/vmstat | rg "refault" >> ./report/rec-custom-$DATE/refault.txt
cat /proc/vmstat | rg "pgsteal" >> ./report/rec-custom-$DATE/pgsteal.txt
cat /sys/module/damon_reclaim/parameters/bytes_reclaimed_regions >> ./report/rec-custom-$DATE/damon_passed.txt
sar -r 5 $((($TEST_MINS*60)/5)) >> ./report/rec-custom-$DATE/memu.txt &
sar -q CPU 5 $((($TEST_MINS*60)/5)) >> ./report/rec-custom-$DATE/cpu.txt &
sar -q IO 5 $((($TEST_MINS*60)/5)) >> ./report/rec-custom-$DATE/io.txt &
sar -q MEM 5 $((($TEST_MINS*60)/5)) >> ./report/rec-custom-$DATE/memo.txt &
sar -B 5 $((($TEST_MINS*60)/5)) >> ./report/rec-custom-$DATE/fault.txt &
